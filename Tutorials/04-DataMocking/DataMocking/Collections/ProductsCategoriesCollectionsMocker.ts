...
class CategoriesCollectionsMocker {
  ...
  public static generate(mockingOrder: CategoriesCollectionsMocker.MockingOrder): Array<ProductCategory> {
    ...
    for (const subset of mockingOrder) {
      ...
      for (let itemNumber: number = 1; itemNumber <= itemsQuantity; itemNumber++) {
        accumulatingCollection.push(ProductCategoryMocker.generate(
